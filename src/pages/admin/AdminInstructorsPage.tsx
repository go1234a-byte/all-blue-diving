...
const AdminInstructorsPage = () => {
  ...
  const filteredInstructors = instructors
    ...
    .sort((a, b) => {
      // 심사 대기중(미인증·미반려)이거나, 이미 인증된 강사가 서류 수정요청을 낸 경우
      // 정렬 조건과 무관하게 항상 맨 위로 올린다. 예전에는 새로 가입한 강사가 목록 맨
      // 뒤에 추가되는 구조라, 승인이 시급한 신청 건이 전체 강사 목록 맨 아래에 파묻혀
      // 관리자가 놓치기 쉬웠다.
      const aPending = (!a.verified && !a.rejectedAt) || (a.verified && a.documentsPendingReview);
      const bPending = (!b.verified && !b.rejectedAt) || (b.verified && b.documentsPendingReview);
              ...
              >
                <div className="flex items-center justify-between gap-2">
                  <div className="flex min-w-0 items-center gap-1.5">
                    ...
                    {instructor.verified && instructor.documentsPendingReview && (
                      <Badge variant="secondary" className="shrink-0 text-[10px]">
                        수정요청
                      </Badge>
                    )}
