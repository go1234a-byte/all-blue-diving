...
begin
  ...
  select refund_amount, refund_rate, coupon_code into v_actual_refund_amount, v_refund_rate, v_coupon_code
  ...
  -- 취소된 예약이 쓴 쿠폰은 한도(used_count)에서 돌려준다 — 안 그러면 취소해도 한정수량
  -- 쿠폰이 영구히 소진된 것으로 남아, 재예약 시 같은 쿠폰을 다시 못 쓰는 문제가 있었다.
  -- cancel_booking_with_refund는 예약당 정확히 한 번만(confirmed 상태에서만) 이 함수를
  -- 부르므로 중복 차감 걱정은 없다.
